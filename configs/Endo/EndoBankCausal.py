_base_ = [
    # './backbone.py',
    '../_base_/datasets/Endo.py',
    '../_base_/schedules/FFA_bs512_100e_coslr_warmup.py',
    '../_base_/default_runtime.py'
]

work_dir = './work_dirs/Endo_BankCausal'
...
model = dict(
    ...
    backbone=dict(type='ConvNeXt', 
                  arch='small',  # Options: tiny, small, base, large
                  drop_path_rate=0.5,
                  gap_before_final_norm=False,
                  init_cfg=dict(type='Pretrained', 
                      prefix='backbone',
                      checkpoint='https://download.openmmlab.com/mmclassification/v0/convnext/convnext-small_32xb128_in1k_20221207-4ab7052c.pth')
                  ),
...
custom_hooks = [
    dict(
        ...
        feat_dim=hidden_dim,
        queue_length=2000,
        frozen_epoch_cfg=dict(prototypes=30),
        loss_decay = dict(
            start_weight=1,
            end_weight=0,    # Final weight
            decay_type='linear', 
            start_epoch=10, # or 'exponential'
            total_epochs=30  # Should match max_epochs)),
        )
    ),
    dict(
    ...
    # dict(
    #     type='LossWeightDecayHook',
    #     param_path='head.swav_loss.weight',  # Path to SwAV loss weight
    #     start_weight=1.0,  # Initial weight
    #     end_weight=0.0,    # Final weight
    #     decay_type='exponential',  # or 'exponential'
    #     total_epochs=100   # Should match max_epochs
    # )
]