...
def merge_args(cfg, args):
    ...
    return cfg
class DumpPredAndGT(DumpResults):
    def process(self, data_batch, predictions):
        results = []
        for pred in predictions:
            result = {}
            # Save prediction
            if 'pred_label' in pred:
                result['pred_label'] = pred['pred_label'].tolist() if hasattr(pred['pred_label'], 'tolist') else pred['pred_label']
            if 'pred_score' in pred:
                result['pred_score'] = pred['pred_score'].tolist() if hasattr(pred['pred_score'], 'tolist') else pred['pred_score']
            # Save ground truth
            if 'gt_label' in pred:
                result['gt_label'] = pred['gt_label'].tolist() if hasattr(pred['gt_label'], 'tolist') else pred['gt_label']
            if 'gt_score' in pred:
                result['gt_score'] = pred['gt_score'].tolist() if hasattr(pred['gt_score'], 'tolist') else pred['gt_score']
...
def main():
    ...
    if args.out and args.out_item in ['pred', None]:
        runner.test_evaluator.metrics.append(
            DumpPredAndGT(out_file_path=args.out))
