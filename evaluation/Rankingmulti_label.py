...
from mmpretrain.evaluation.metrics.single_label import _precision_recall_f1_support, to_tensor
from mmpretrain.evaluation.metrics.multi_label import _average_precision
from sklearn.metrics import *


@METRICS.register_module()
class RankingMultiLabelMetric(BaseMetric):
    ...
    **Support** is the number of samples:
    ...
    def process(self, data_batch, data_samples: Sequence[dict]):
        ...
        for data_sample in data_samples:
            ...
            # if self.Ranking:
            #     result['disease_count'] = data_sample['disease_count'].clone()
    ...
    def calculate(
        ...
        # disease_count:Union[torch.Tensor, np.ndarray, Sequence]=None,
        ...
        # if disease_count is not None:
        #     pos_inds = []
        #     for i in range(pred.size(0)):  # Iterate over each row
        #         k = torch.sigmoid(disease_count[i]).item()  # Extract the number of top elements from disease_count for this row
        #         topk_idx = torch.where(pred[i] >= k)[0]
        #         # _, topk_idx = pred[i].topk(k)  # Get the top-k indices for the current row
        #         # topk_indices.append(topk_idx)
        #         pos_ind = torch.zeros_like(pred[i]).scatter_(0, topk_idx, 1)
        #         pos_inds.append(pos_ind)
               
        #     # _, topk_indices = pred.topk(disease_count.item())
        ...
        pos_count = target.sum(dim=0)
        # Step 2: Get indices of valid classes (>=2 positives)
        valid_indices = (pos_count >= 2).nonzero(as_tuple=True)[0]

        # Step 3: Filter both target and pred
...
@METRICS.register_module()
class RankingAverageAUC(BaseMetric):
    default_prefix: Optional[str] = 'multi-label'
    ...
    def calculate(
        ...
        Args:
            pred (torch.Tensor | np.ndarray | Sequence): The prediction
                ...
                format labels.
            target (torch.Tensor | np.ndarray | Sequence): The prediction
                results. A :obj:`torch.Tensor` or :obj:`np.ndarray`batch_labels = [ds.gt_score for ds in data_batch['data_samples'][0]] with
                shape ``(N, num_classes)`` or a sequence of index/onehot
                format labels.
            pred_indices (bool): Whether the ``pred`` is a sequence of
                category index labels. If True, ``num_classes`` must be set.
        ...
        return roc_auc_score(target_filtered, pred_filtered, average=average)
  
@METRICS.register_module()
class RankingmulticlassAverageAUC(BaseMetric):
...
@METRICS.register_module()
class RankingROC_AUC(BaseMetric):
    default_prefix: Optional[str] = 'multi-class'
    ...
    def compute_metrics(self, results: List):
        ...
        if self.average is None:
            ...
            # result_metrics['AUC_multiclasswise'] = np.mean(auc_list.copy())
        else:
            result_metrics['mAUC_multiclass']=np.mean(auc_list.copy())
...
@METRICS.register_module()
class MultiLabel_AveragePrecision(BaseMetric):
...
@METRICS.register_module()
class MultiLabel_Accuracy(BaseMetric):
    r"""Calculate the average accuracy with respect of classes.

    AverageAccuracy (AA) summarizes the accuracy for each class and calculates
    ...
    def __init__(self,
                 ...
                 collect_device: str = 'cpu',
                 Validation_mode: str = 'original',
                 task: str = 'multi-label', # 'multi-label' or 'multi-class' or 'binary-class'
                 prefix: Optional[str] = None) -> None:
        super().__init__(collect_device=collect_device, prefix=prefix)
        self.average = average
        self.Validation_mode = Validation_mode
    ...
    def compute_metrics(self, results: List):
        ...
        acc = self.calculate(pred, target, self.average,self.task,self.threshold) 

        result_metrics = dict()


        result_metrics['accuracy'] = acc
    ...
    def calculate(pred: Union[torch.Tensor, np.ndarray],
        ...
        if task == 'binary-class':
            ...
            ret = accuracy_score(target, pred > threshold)
        if task == 'multi-class':
            ret = accuracy_score(target.argmax(dim=1), pred.argmax(dim=1))
        if task == 'multi-label':
            pred = (pred >= threshold).long()
            target = target.long()
            acc = 0
            for i in range(pred.shape[1]):
                label_acc = accuracy_score(target[:, i], pred[:, i])
                acc += label_acc
            ret = acc / pred.shape[1]
        return ret*100
