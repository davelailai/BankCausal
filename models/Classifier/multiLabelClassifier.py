...
from mmpretrain.models import BaseClassifier
from mmengine.optim import OptimWrapper


@MODELS.register_module()
class MultiLabelImageClassifier(BaseClassifier):
    ...
    def __init__(self,
        ...
        if isinstance(data_preprocessor, dict):
            data_preprocessor.setdefault('type', 'MultiLabelClsDataPreprocessor')
    ...
    def train_step(self, data: Union[dict, tuple, list],
                   optim_wrapper: OptimWrapper, **kwargs) -> Dict[str, torch.Tensor]:
        ...
        #         print(f'{n}: grad mean={p.grad.abs().mean().item()}')
        #     else:
    ...
    def predict(self,
        ...
        try:
            ...
            kwargs['mean'] = self.data_preprocessor.mean
            kwargs['std'] = self.data_preprocessor.std
        except AttributeError as e:
            print(f"[Warning] Failed to extract preprocessing info: {e}")
        ...
        # Safely extract data sample attributes
        # for item in data_samples:
        #     try:
        #         gt_score.append(item.gt_score)
        #     except AttributeError:
        #         gt_score.append(None)  # or skip, or raise warning
        ...
        #     try:
        #         original_shape.append(item.ori_shape)
        #     except AttributeError:
        #         original_shape.append(None)

        # # Update kwargs
        # kwargs['gt_score'] = gt_score
        # kwargs['img_path'] = img_path
        # kwargs['ori_shape'] = original_shape
...
@MODELS.register_module()
class MultiLabelImageGanClassifier(BaseClassifier):
    """Image classifiers for supervised classification task.
    ...
    def __init__(self,
        ...
        super(MultiLabelImageGanClassifier, self).__init__(
            init_cfg=init_cfg, data_preprocessor=data_preprocessor)
    ...
    def train_step(self, data: Union[dict, tuple, list],
        ...
        if kwargs['current_epoch']< self.warmup_epoch:
            self.set_requires_grad(self.head.task_heads.classification.DagmaMLP, False)
            self.head.task_heads.classification.feature_reg=False
            # base_optimizer_wrapper = optim_wrapper['backbone']
            base_optimizer_wrapper=optim_wrapper
            with base_optimizer_wrapper.optim_context(self):
                data = self.data_preprocessor(data, True)
                losses = self._run_forward(data, mode='loss')  # type: ignore
                losses.pop('classification_causal_loss')
                # losses.pop('causal_loss')
                parsed_losses, log_vars = self.parse_losses(losses)  # type: ignore
                base_optimizer_wrapper.update_params(parsed_losses)
            self.set_requires_grad(self.head.task_heads.classification.DagmaMLP, True)
            return log_vars
        else:
            # self.head.task_heads.classification.feature_reg=True
            # self.set_requires_grad(self.head, False)
            # self.set_requires_grad(self.backbone, False)
            # self.set_requires_grad(self.head.task_heads.classification.DagmaMLP, True)
            # self.set_requires_grad(self.head.fc, True)
            # causal_optimizer_wrapper = optim_wrapper['head']
            causal_optimizer_wrapper=optim_wrapper
            with causal_optimizer_wrapper.optim_context(self):
                data = self.data_preprocessor(data, True)
                losses = self._run_forward(data, mode='loss')  # type: ignore
                # losses.pop('cls_loss')
                # if 'alignmnet_loss' in losses:
                #     losses.pop('alignmnet_loss')
                parsed_losses, log_vars = self.parse_losses(losses)  # type: ignore
                causal_optimizer_wrapper.update_params(parsed_losses)
            # self.set_requires_grad(self.head, True)
            # self.set_requires_grad(self.backbone, True)
    ...
    def predict(self,
        ...
        # kwargs['mean']=mean
        # kwargs['std']=std
        # # kwargs['data_samples'] = data_samples
        # gt_score=[]
        # img_path = []
        # original_shape = []
        # for item in data_samples:
