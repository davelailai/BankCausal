...
class DagmaMLP(nn.Module): 
    ...
    def __init__(self, dims: typing.List[int], disease_num: int, bias: bool = True, dtype: torch.dtype = torch.double, causal_part: str='Whole'):
        r"""
        Parameters
        ----------
        ...
        if causal_part=='Whole':
            self.I = torch.eye(self.d)
        if causal_part=='Disease':
            self.I = torch.eye(self.d)
            self.I[disease_num:,disease_num:]=1

        # if causal_part=='Disease_only':
        #     self.I = torch.eye(self.d)
    ...
    def h_func(self, s) -> torch.Tensor:
        ...
        # sign, logabsdet = torch.slogdet(s * self.I - A)
    ...
    def fc1_l1_reg(self,num_class) -> torch.Tensor:
        r"""
        ...
        """
        fc1_weight = torch.abs(self.fc1.weight)
        fc1_weight = fc1_weight.view(self.d, -1, self.d) 
        fc1_weight = fc1_weight.permute(0,2,1)
        C1 = torch.sum(torch.abs(fc1_weight[num_class:,num_class:,:]))
        # C2 = -torch.sum(torch.log(fc1_weight[num_class:,:]))
        return torch.sum(torch.abs(self.fc1.weight)) +C1
    
    # def fc1_l1_reg_C1(self,disease_num) -> torch.Tensor:
    #     r"""
    #     Takes L1 norm of the weights in the first fully-connected layer
    ...
    #     fc1_weight = fc1_weight.view(self.d, -1, self.d) 
    #     fc1_weight = fc1_weight.permute(0,2,1)
    #     return torch.sum(fc1_weight[0:disease_num,disease_num:,:])
